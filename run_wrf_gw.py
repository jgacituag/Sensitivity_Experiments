...
conf['modify_wind_proflie'] = True  #Are we going to modify the original wind profile?
conf['remove_mean_wind']    = False #True for convection experiments, false for mountain wave experiments.
conf['shear_depth_u']    = 2000.0   #Heigth of no shear (for the u component)
...
conf['modify_theta_proflie'] = True  #Are we going to modify the original wind profile?
conf['surface_theta'] = 288.0        #Potential temperature at the lowest model level.
#Temperature profile will be computed as costant d(theta)/dz in user defined layers. 
conf['theta_layer_limits']  = np.array([0.0,3000.0,10000.0])  #Layer limits.
conf['dthetadz_layer']      = np.array([4.0,0.4,10.0])/1000.0 #dthetadz corresponding to each layer.


#Parameters controling the model parameters (parameters controlling the model integration)
conf['model_xdomain'] = 2000.0 * 202  #Do not change (total extension of the domain in the x direction, meters)
...
conf['model_dt'] = 20                #Integration time step (seconds, integer)            
...
conf['sf_type'] = 'near_surface_windspeed' 
